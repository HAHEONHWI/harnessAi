#!/usr/bin/env bash
# Installs the latest AI Harness release from GitHub:
...
curl -fsSL -o "$tmp/app.zip" "https://github.com/$repo/releases/latest/download/AI-Harness-macOS.zip"
ditto -x -k "$tmp/app.zip" "$tmp"
rm -rf "$install_dir/AI Harness.app"
mv "$tmp/AI Harness.app" "$install_dir/"
# The app is ad-hoc signed, not notarized; clear the download quarantine so Gatekeeper allows it.
xattr -dr com.apple.quarantine "$install_dir/AI Harness.app" 2>/dev/null || true

engine_dir="$HOME/.ai-harness/bin"
mkdir -p "$engine_dir"
install -m 755 "$install_dir/AI Harness.app/Contents/Resources/harness.py" "$engine_dir/harness.py"
